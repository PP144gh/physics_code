...
with open('b2.txt','r') as csvfile:
        ...
        for row in plots:
                ...
                print(row[4])
                if(float(row[10])==0):
                        listx.append(float(row[4]))
                        listy.append(float(row[7]))
                if(float(row[10])==1):
                        listx2.append(float(row[4]))
                        listy2.append(float(row[7]))
...
#plt.yscale('log')
#plt.xscale('log')
plt.xlim([float(0.01),float(0.055)])
plt.ylim([float(1.10),float(1.3)])
...
listpl=[]
#MUDAR NOME
plt.title((r'$Correlation~34$'))
#plt.axhline(y=-3.25, color='lawngreen', linestyle='-')
#MUDAR NOME
plt.xlabel(r'$\theta_{14}$')
...
#MUDAR NOME
fig.savefig("plot_34.jpg",quality=100)
plt.show()
