...
with open('b2.txt','r') as csvfile:
        ...
        for row in plots:
                ...
                if(float(row[10])==0):
                        ...
                        listy.append(float(row[6]))
                if(float(row[10])==1):
                        listx2.append(float(row[1]))
                        listy2.append(float(row[6]))
                '''
                listx.append(float(row[1]))
                listy.append(float(row[6]))
...
#plt.yscale('log')
#plt.xscale('log')
plt.xlim([float(0.22), float(0.235)])
plt.ylim([float(0.),float(0.02)])
...
#MUDAR NOME
fig.savefig("plot_41.jpg",quality=100)
plt.show()
