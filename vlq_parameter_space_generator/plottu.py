...
with open('b2lenz.txt','r') as csvfile:
        ...
        for row in plots:
                ...
                listy.append(float(row[11])*10**(-8))
                listx.append(math.sin(float(row[4])))
                '''
                if(float(row[10])==0):
                        listy.append(float(row[11])*10**(-8))
                        listx.append(math.sin(float(row[4])))
                if(float(row[10])==1):
                        listy2.append(float(row[11])*10**(-8))
                        listx2.append(math.sin(float(row[4])))
                '''
...
plt.yscale('log')
#plt.xscale('log')
plt.xlim([float(0.0), float(0.05)])
#plt.ylim([float(0.01),float(0.05)])
...
#MUDAR NOME
fig.savefig("plot_Btu.jpg",quality=100)
plt.show()
